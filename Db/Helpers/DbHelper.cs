        ...
        private static void GetProcedureSchema(Procedure procedure, SqlConnection connection)
        {
            using (SqlCommand command = new SqlCommand())
            {
                ...
                using (SqlDataReader reader = command.ExecuteReader(CommandBehavior.SchemaOnly))
                {
                    ...
                    if (schemaTable != null)
                    {
                        procedure.ProcedureColumns = schemaTable.ToList<ProcedureColumn>((string columnName, Type columnType, object value) =>
                        {
        ...
        private static void GetFunctionSchema(Function function, SqlConnection connection)
        {
            using (SqlCommand command = new SqlCommand())
            {
                ...
                using (SqlDataReader reader = command.ExecuteReader(CommandBehavior.SchemaOnly))
                {
                    ...
                    if (schemaTable != null)
                    {
                        function.ProcedureColumns = schemaTable.ToList<ProcedureColumn>((string columnName, Type columnType, object value) =>
                        {
                            if (columnName == "NumericScale")
                                return (int?)(value as short?);
                            return value;
